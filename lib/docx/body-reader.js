...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "m:oMathPara": readMathElement,
        "m:oMath": readMathElement
    };
...
function readMathElement(element) {
    function transform(data) {
        function _transform(data) {
            var name = data.name;
            if (!name) {
                return new xmldom.DOMImplementation().createDocument().createTextNode(data.value);
            }
            var tagName = name.replace(/\{.*\}/, 'm:');
            var el = new xmldom.DOMImplementation().createDocument().createElement(tagName);
    ...
    var doc = new xmldom.DOMParser().parseFromString(new xmldom.XMLSerializer().serializeToString(om));
    var math = omml2mathml(doc);
    return elementResult(new documents.Run([new documents.Raw(math.outerHTML)], {
        'styleName': 'math'
    }));
}
